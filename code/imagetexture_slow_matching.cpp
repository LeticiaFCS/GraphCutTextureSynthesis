...
std::pair<int, int> ImageTexture::matching(const png::image<png::rgb_pixel> &inputImg){
    ...
    {    //calc stdDev of input img
        ...
        long double pixelCnt = inputImg.get_height() * inputImg.get_width();
        for(int i = 0; i < (int) inputImg.get_height(); i++)
            for(int j = 0; j < (int) inputImg.get_width(); j++){
                avgRed += inputImg[i][j].red;
                avgGreen += inputImg[i][j].green;
                avgBlue += inputImg[i][j].blue;
            }
        avgRed /= pixelCnt;
        avgGreen /= pixelCnt;
        avgBlue /= pixelCnt;
        ...
        for(int i = 0; i < (int) inputImg.get_height(); i++)
            for(int j = 0; j < (int) inputImg.get_width(); j++){
                ...
                varGreen += pow2(avgGreen - inputImg[i][j].green);
                varBlue += pow2(avgBlue - inputImg[i][j].blue);
            }
        varRed /= pixelCnt;
        varGreen /= pixelCnt;
        varBlue /= pixelCnt;
        ...
        for(int widthOffset = - imgWidth + 1; widthOffset < imgWidth; widthOffset++){
            ...
            for(int i = 0, a = i + heightOffset; i < (int) inputImg.get_height() && a < this->imgHeight; i++, a++){
                for(int j = 0, b = j + widthOffset; j < (int) inputImg.get_width() && b < this->imgWidth; j++, b++){
                    if(a < 0 || b < 0)
                        continue;
                    if(pixelColorStatus[a][b] == PixelStatusEnum::notcolored)
                        continue;
                    A++;
                    cost += pow2(inputImg[i][j].red - outputImg[a][b].red);
                    cost += pow2(inputImg[i][j].green - outputImg[a][b].green);
                    cost += pow2(inputImg[i][j].blue - outputImg[a][b].blue);
                }
            }
